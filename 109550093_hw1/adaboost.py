...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        Selects the best weak classifier for the given weights.
          Parameters:
            hj:featureVals: A numpy array of shape (len(features), len(dataset)).
              Each row represents the values of a single feature for each training sample.
            x:iis: A list of numpy array with shape (m, n) representing the integral images.
            y:labels: A list of integer.
        ...
        bestError=10000000
        Error = 0
        bestClf = WeakClassifier(-1)
        for j in range(len(featureVals)):
            Error = 0
            #print("length:",len(featureVals[j]))
            for i in range(len(featureVals[j])):
                #print(featureVals[j,i])
                if featureVals[j,i]>=0:
                    temp =0
                else:
                    temp =1
                
                Error+=(weights[i]*abs(temp-labels[i]))
                #print(Error)
            if Error<bestError:
                bestError = Error
                #print(bestError)
